...
impl DataSeriesEncodings {
    pub fn len(&self) -> usize {
        ...
        if self.bam_flags().is_some() {
            n += 1;
        }

        if self.cram_flags().is_some() {
            n += 1;
        }
        ...
        if self.alignment_starts().is_some() {
            n += 1;
        }

        if self.read_group_ids().is_some() {
            n += 1;
        }
...
mod tests {
    ...
    fn test_len() {
        ...
        let map = DataSeriesEncodings::init();
        assert_eq!(map.len(), 28);

        let map = DataSeriesEncodings {
            bam_flags: Some(Encoding::new(Integer::External {
                block_content_id: 1,
            })),
            cram_flags: Some(Encoding::new(Integer::External {
