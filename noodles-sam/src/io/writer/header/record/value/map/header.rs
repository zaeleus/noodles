    ...
    W: Write,
{
    ...
    if let Some(sort_order) = header.sort_order() {
        fmt_display_field(writer, tag::SORT_ORDER, sort_order)?;
    }

    if let Some(group_order) = header.group_order() {
        fmt_display_field(writer, tag::GROUP_ORDER, group_order)?;
    }

    if let Some(subsort_order) = header.subsort_order() {
        fmt_display_field(writer, tag::SUBSORT_ORDER, subsort_order)?;
    }
