...
mod tests {
    ...
    fn test_from_type_for_symbol() {
        let symbol = StructuralVariant::from(Type::Deletion);
    ...
    fn test_from_str() -> Result<(), ParseError> {
        let actual: StructuralVariant = "DEL".parse()?;
        assert_eq!(actual.ty(), Type::Deletion);
        assert!(actual.subtypes().is_empty());

        let actual: StructuralVariant = "DEL:TANDEM".parse()?;
        assert_eq!(actual.ty(), Type::Deletion);
        assert_eq!(actual.subtypes(), &[String::from("TANDEM")]);

        let actual: StructuralVariant = "DEL:ME:ALU".parse()?;
        assert_eq!(actual.ty(), Type::Deletion);
        assert_eq!(
            actual.subtypes(),
        ...
        assert!("".parse::<StructuralVariant>().is_err());
