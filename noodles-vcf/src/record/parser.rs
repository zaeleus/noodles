...
use super::{
    ...
    reference_bases, AlternateBases, Field, Filters, Genotypes, Ids, Info, QualityScore, Record,
    ReferenceBases, FIELD_DELIMITER, MISSING_FIELD,
};
use crate::Header;
...
pub fn parse(s: &str, header: &Header) -> Result<Record, ParseError> {
    ...
    let r#ref = parse_reference_bases(&mut fields)?;
...
fn parse_reference_bases<'a, I>(fields: &mut I) -> Result<ReferenceBases, ParseError>
where
    I: Iterator<Item = &'a str>,
{
    parse_string(fields, Field::ReferenceBases).and_then(|s| match s {
        MISSING_FIELD => Err(ParseError::MissingField(Field::ReferenceBases)),
        _ => s.parse().map_err(ParseError::InvalidReferenceBases),
    })
}

fn parse_alternate_bases<'a, I>(fields: &mut I) -> Result<AlternateBases, ParseError>
where
    I: Iterator<Item = &'a str>,
{
    parse_string(fields, Field::AlternateBases).and_then(|s| match s {
        MISSING_FIELD => Ok(AlternateBases::default()),
        _ => s.parse().map_err(ParseError::InvalidAlternateBases),
    })
}
...
mod tests {
    ...
    fn test_from_str_with_genotype_info() -> Result<(), Box<dyn std::error::Error>> {
        use self::genotypes::values::Value;
